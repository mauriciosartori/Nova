 ...
 * Output = (1,5), (6,9)
 */
class MergeIntervals2Elaborated {
    companion object {
        @JvmStatic
    ...
    private class IntervalTime(val time: Int, val isStartTime: Boolean): Comparable<IntervalTime> {
        override fun compareTo(other: IntervalTime): Int {
            return if (time == other.time) {
                if (isStartTime == other.isStartTime) {
                    0
                } else {
                    if (isStartTime) -1 else 1
                }
            } else {
                if (time > other.time) 1 else -1
            }
        }
