 ...
 */
class SingleBinarySearch {
    companion object {
        @JvmStatic
        fun main(args: Array<String>) {
            val inputArray = intArrayOf(1,3,5,6,9,11,14,17,1,21)
            //val target = 9
            val target = 15
            ...
            while (start <= end) {
                ...
                when {
                    ...
                    inputArray[middle] < target -> start = middle + 1
                            ...
                            println("We found the target $target at position $middle")
                            return
                        }
                    }
                }
            println("Sorry bro there is not target $target at this time")
        }
    }
}